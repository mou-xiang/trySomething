...
| **M3** | 🔄 **进行中** | 网络理论已学完（DNS/TCP/HTTP 全链路 + nslookup 实验）；**待：买域名 + 买香港轻量服务器 + 实名** |
...
1. **【进行中】腾讯云学生套餐评估**（cloud.tencent.com/act/campus）：
   - 关键：页面地域选择器**能否选"中国香港"**
   - 能选 → 2核2G 学生价（~10-15 元/月）直接买 ✅
   - 只有大陆地域 → **放弃学生套餐**（大陆=要备案，与用户约束冲突），改买**普通香港轻量**（~24-38 元/月，年付促销 50~150 元/年）
   - 陷阱：学生价续费会变回原价（毕业/每年重新认证），注意对比年付促销
2. **买域名**：和服务器同厂商（腾讯云 DNSPod / 阿里云万网），首年 10~60 元，需域名实名（几小时~1天，≠ 备案）
3. **买服务器后保存**：公网 IP、登录密码/密钥（M4 要用）
